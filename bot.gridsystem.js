    ...
    return {
        ...
        gridLineAntiAlias: function(x0, y0, x1, y1, wd, cellData) {
            ...
            for (wd = (wd+1)/2; ; ) {                                   /* pixel loop */
                    if( !(x0 < 0 || x0 > collisionGrid.gridWidth-1 || y0 < 0 || y0 > collisionGrid.gridHeight-1) ) {
                        node = collisionGrid.getCell(x0,y0);
                        collisionGrid.markCellWall(node, window.botsettings.astarSnakeMaxWeight*Math.max(0,(Math.abs(err-dx+dy)/ed-wd+1)), cellData);
                    }
                ...
                if (2*e2 >= -dx) {                                           /* x step */
                    for (e2 += dy, y2 = y0; e2 < ed*wd && (y1 != y2 || dx > dy); e2 += dx) {
                        y2 += sy;
                        if( !(x0 < 0 || x0 > collisionGrid.gridWidth-1 || y2 < 0 || y2 > collisionGrid.gridHeight-1) ) {
                            node = collisionGrid.getCell(x0, y2);
                            collisionGrid.markCellWall(node, window.botsettings.astarSnakeMaxWeight*Math.max(0,(Math.abs(e2)/ed-wd+1)), cellData);
                        }
                ...
                if (2*e2 <= dy) {                                            /* y step */
                    for (e2 = dx-e2; e2 < ed*wd && (x1 != x2 || dx < dy); e2 += dy) {
                        x2 += sx;
                        if( !(x2 < 0 || x2 > collisionGrid.gridWidth-1 || y0 < 0 || y0 > collisionGrid.gridHeight-1) ) {
...
var collisionGrid = (function() {
    ...
    return {
        ...
        drawSubGrid: function(startCol, startRow, endCol, endRow, color) {
            ...
            startCol = Math.min(Math.max(startCol, 0), collisionGrid.gridWidth-1);
            startRow = Math.min(Math.max(startRow, 0), collisionGrid.gridHeight-1);

            endCol = Math.min(Math.max(endCol, 0), collisionGrid.gridWidth-1);
            endRow = Math.min(Math.max(endRow, 0), collisionGrid.gridHeight-1);
        ...
        addFood: function() {
            ...
            for( var i=0; i<collisionGrid.foodGroups.length; i++) {
                ...
                if( foodgroup.score >= window.botsettings.foodHighQualityScore ) {
            ...
            if( window.visualDebugging ) {
                ...
                for( var i=0; i<collisionGrid.foodGroups.length; i++) {
                    ...
                    for( var j=0; j<foodgroup.nodes.length; j++) {
                        var foodpos = {x: foodgroup.nodes[j].x, y: foodgroup.nodes[j].y};
        ...
        addSnakes: function() {
            ...
            window.snake.snkWidth = window.getSnakeWidth();
            window.snake.snkWidth2 = window.snake.snkWidth * 2;
            window.snake.snkWidthSqr = window.getSnakeWidthSqr();
            ...
            for (snakeid in window.snakes) {
                ...
                snk.snkWidth2 = window.snake.snkWidth * 2;
                ...
                threat = collisionGrid.setupSnakeThreatRadius(snk, window.botsettings.collisionSnakePartSizeMultiplier);
                otherSnkWidthSqr = window.getSnakeWidthSqr(snk);
                otherSnkWidth = window.getSnakeWidth(snk.sc);
                otherSnkWidth2 = otherSnkWidth*2;
                var radius = collisionGrid.calculateMaxCellCount(otherSnkWidth)+2;
                var lastpts = snk.pts.length-1;
                var lastPos = 0;

                //add all the parts + some dying (behind snake)
                for (pts=lastpts; pts>=3; pts-=3) {// in snk.pts) {
                    part = snk.pts[pts];
                    pts2 = pts-3;
                    //part2 = snk.pts[pts2];
                    ...
                    if( lastPos ) {
                        var dist2 = canvas.getDistance2(lastPos.x, lastPos.y, part.xx, part.yy);
...
/*
        ...
        snakePartBounds: function(snk, part, threatLevels, snkWidthSqr, otherSnkWidthSqr) {
            ...
            var maxdist = astar.heuristics.manhattan({x:cellpos.col, y:cellpos.row}, {x:cellpos.col-maxthreat, y:cellpos.row-maxthreat});
            ...
            collisionGrid.sliceGrid(cellpos.col-maxthreat, cellpos.row-maxthreat, maxthreat2, maxthreat2,
                function(col, row, node) {
                    ...
                    var dist2 = astar.heuristics.manhattan({x:cellpos.col, y:cellpos.row}, {x:col, y:row});//canvas.getDistance2(cellpos.col, cellpos.row, col, row);

                    var cellData = {snake:snk, part:part};
                    var weight = 1-Math.max(0.01, Math.min(1.0,dist2/maxdist));
                    weight = weight * weight;
                    collisionGrid.markCellWall(node, window.botsettings.astarSnakeMaxWeight*weight, cellData);
                    if( window.gridDebugging )
                    collisionGrid.drawCell(col, row, 'rgba(0,255,255,' + weight + ')' );
