...
var bot = (function() {
    ...
    return {
        ...
        radarResults: [],
        followLine: 0,
        behaviorData: {foodTarget:0, followTime:0, followCoordinates:{x:0,y:0}, goalCoordinates:{x:0,y:0}, aggressor:0, manualSprint:false },
        ...
        changeOtherSkin: function() {
            if( window.playing &&  window.snakes.length > 0) {
                var newskin = window.snake.rcv;
                var max = window.max_skin_cv;
                if (newskin > max) {
                    newskin = 0;
                }
                for (snakeid in window.snakes) {
                    window.setSkin(window.snakes[snakeid], newskin);
                }
            }
        },
    ...
    return {
        ...
        appendSettings: function(parentElem) {
            ...
            for(var key in window.botsettings ) {
                ...
                div.innerHTML = '<label style="margin-top:8px; color:#bbb; font-size:11px; display:block;width:200px">' + key + "</label>" + '<input style="font-size:10px; background-color:#111; color:#777; height:16px; border:1px solid #333; line-height:16px;" type="text" id="' + key + '" value="' + value + '">';
                parentElem.appendChild(div);
            }

            var div = document.createElement('span');
            div.className = 'botSaveSettings';
            div.innerHTML = '<input style="margin-top:10px; background-color:#222; border:0; color:#ccc;" type="button" id="saveBotSettings" value="Save">';
            div.onclick = userInterface.saveSettings;
            parentElem.appendChild(div);

            var div = document.createElement('span');
            div.className = 'botResetSettings';
            div.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input style="margin-top:10px; background-color:#222; border:0; color:#ccc;"  type="button" id="resetBotSettings" value="Defaults">';
        ...
        onmouseup: function(e) {
            e = e || window.event;
            if( original_onmouseup )
                original_onmouseup(e);
            if( window.playing) {
                switch(e.which) {
                    case 1:
...
(function() {
    ...
    userInterface.appendDiv('changeskin_overlay', 'nsi', window.generalstyle +
        'left: 30; top: 200px;');
    userInterface.appendDiv('changeotherskin_overlay', 'nsi', window.generalstyle +
        'left: 30; top: 200px;');
    ...
    window.changeskin_overlay.innerHTML = window.spanstyle +
        '(X) Change skin </span>';
    window.changeotherskin_overlay.innerHTML = window.spanstyle +
        '(C) Change others skin </span>';
